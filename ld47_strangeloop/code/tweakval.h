...
// supports it, use it
#if TV_USE_TWEAKVAL
#  define _TV(Val) TweakValueFloat( __FILE__, __COUNTER__, Val )
#  define _TVI(Val) TweakValueInt( __FILE__, __COUNTER__, Val )

//extern "C" {
float TweakValueFloat( const char *file, size_t counter, float origVal );
int TweakValueInt( const char *file, size_t counter, int origVal );
