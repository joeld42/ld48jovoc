...
void UIAssets::SetupUI()
{
    ...
    IO::Load("gamedata:uiassets/Roboto-Medium.ttf", [this](IO::LoadResult loadResult) {
        ...
        this->font_30 = NKUI::AddFont(this->ttfData, 40.0f);
        NKUI::EndFontAtlas();
        ...
        LoadIcon( "gamedata:uiassets/btn-big-dots.png", &img_btn_big_dots );
        
        
        LoadIcon( "gamedata:uiassets/title_card.png", &img_title_card );
...
void UIAssets::buttonStyleBigDots( nk_context* ctx ) {
    ctx->style.button.normal = nk_style_item_image( img_btn_big_dots );
    ctx->style.button.hover = nk_style_item_image( img_btn_big_dots );
    ctx->style.button.text_normal = nk_rgb(20, 50, 60);
    ctx->style.button.text_hover = nk_rgb(100, 200, 130);
    ctx->style.button.padding = nk_vec2(0,0);
}
