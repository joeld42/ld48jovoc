...
glm::vec3 Camera::worldPointToScreen2( glm::vec3 worldPnt )
{
    glm::vec4 ndcPoint = worldPointToNDC( worldPnt );
    ...
    glm::vec2 ndcScreen( ndcPoint );
    ndcScreen = (ndcScreen + glm::vec2( 1.0 )) * 0.5f;
    return glm::vec3( ndcScreen.x * displayWidth, (1.0-ndcScreen.y) * displayHeight,
                     ndcPoint.z  );

}


glm::vec2 Camera::screenToNDC( glm::vec2 screenPnt )
{
    glm::vec2 ndcScreen = glm::vec2( ((screenPnt.x + 0.5f) * 2.0) * displayWidth,
                                     ((screenPnt.y + 0.5f) * 2.0) * displayHeight );
    return ndcScreen;
}
