...
SceneObject *makeObject( SceneMesh *mesh )
{
    ...
    object->vsParams.checkColor = glm::vec4( 0.9,0.6,0,1); // TODO: animate me
//    object->tileVSParams.tintColor = glm::vec4(1);
//    object->tileFSParams.decalColor = glm::vec4(1,1,0,1);
    object->fsParams.decalUVOffs = glm::vec2( 0.0, 0.0 );
    object->fsParams.lightDir = glm::vec3( 0.0, 0.0, 1.0 );
//
...
void Scene::Setup( Oryol::GfxSetup *gfxSetup, int renderSampleCount )
{
    ...
    // Load test texture
    decalBluePrint.Sampler.MinFilter = TextureFilterMode::Nearest;
    decalBluePrint.Sampler.MagFilter = TextureFilterMode::Nearest;
...
//    boardIconTexture = Gfx::LoadResource(TextureLoader::Create(TextureSetup::FromFile( "tx:board_icons.dds", decalBluePrint)));
    ...
    //defaultTexture =
    texEdge = Gfx::LoadResource(TextureLoader::Create(TextureSetup::FromFile( "gamedata:edges.dds", decalBluePrint)));
}
...
//                   glm::length( chunkInfo->m_bboxMin ), chunkContent->m_triIndices/3 );
            ...
            meshSetup.VertexUsage = Usage::Dynamic; // FIXME: only puzzle mesh
            ...
            // Keep the mesh data around (TODO: only for the puzzle mesh)
            mesh->meshDataSize = meshDataSize;
            // Note: this also includes the index buffer so it's not all vertex meshes
            mesh->meshVertData = reinterpret_cast<LDJamFileVertex*>(Memory::Alloc( meshDataSize ));
            memcpy( mesh->meshVertData, meshVertData, meshDataSize );
            mesh->meshIndex = (uint16_t*)((uint8_t*)mesh->meshVertData + meshSetup.IndexDataOffset);
            mesh->numVerts = numVerts;
            mesh->numTris = meshContent->m_triIndices / 3;
...
{
    const auto resEdgeTexState = Gfx::QueryResourceInfo( texEdge ).State;
    if (resEdgeTexState == ResourceState::Valid) {
        this->sceneDrawState.FSTexture[WorldShader::edgeTex] = texEdge;
    }
    ...
    for (int i=0; i < sceneObjs.Size(); i++) {
        ...
        if (resStateMesh == ResourceState::Valid) {
            ...
            obj->fsParams.lightDir = lightDir;
            Gfx::ApplyUniformBlock( obj->fsParams);
