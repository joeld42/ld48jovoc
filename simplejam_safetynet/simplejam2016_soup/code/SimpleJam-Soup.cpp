...
//    }
    ...
    const auto resState3 = Gfx::QueryResourceInfo(this->woodTexture).State;
    if (resState3 == ResourceState::Valid) {
        this->mainDrawState.FSTexture[Textures::Texture] = this->woodTexture;
    }
    
    Gfx::ApplyDefaultRenderTarget(this->mainClearState);
    if (this->mainDrawState.Pipeline.IsValid()) {
        
        this->drawMeshAndTextureIfLoaded( meshBowl, texture );
        this->drawMeshAndTextureIfLoaded( meshTable, woodTexture );
...
//    auto dispPipSetup = PipelineSetup::FromLayoutAndShader(meshSetup.Layout, dispShader);
    ...
    texture =  Gfx::LoadResource(TextureLoader::Create(TextureSetup::FromFile("tex:soupbowl.dds", texBluePrint)));
    woodTexture =  Gfx::LoadResource(TextureLoader::Create(TextureSetup::FromFile("tex:wood.dds", texBluePrint)));
...
//    this->loadMesh( "msh:bowl.omsh" );
    this->meshBowl = Gfx::LoadResource(MeshLoader::Create(MeshSetup::FromFile( "msh:bowl.omsh"), [this](MeshSetup &setup) {
        printf("LOADMESH  BOWL finish block");
    ...
    this->meshTable = Gfx::LoadResource(MeshLoader::Create(MeshSetup::FromFile( "msh:table.omsh"), [this](MeshSetup &setup) {
        printf("LOADMESH  TABLE finish block");
    }));
