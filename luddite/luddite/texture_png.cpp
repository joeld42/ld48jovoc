...
// the nvidia SDK, shader/nv_png.cpp
GLuint Texture_load_PNG_rgba( (const char * filename )
{
	FILE * fp;
    png_byte sig[8];
	...
	DBG::info( "in Texture_load_PNG_rgba %s\n",filename );
    ...
    if ( channels == 3 )
    {
		DBG::error( "Unable to open image of type RGB \"%s\" using read_png_rgba().\n", filename);		
		return;
    }
	...
	DBG::debug( "Loading texture width %d height %d\n", w, h );
...
GLuint Texture_load_PNG_rgba(const char * filename )
{
    ...
    if (!(fp = fopen(filename,"rb")))
	{
		DBG::error( "Cannot open \"%s\" for read!\n", filename);
		return;
	}
    ...
    if ( channels == 4 )
    {
		DBG::error( "Unable to open image of type RGBA \"%s\" using read_png_rgb().\n", filename);		
		return;
    }
